...
class Tool(Tools):
    def __init__(self) -> None:
        ...
        self.update(
            self.load_skill,
            {
                ...
                "function": {
                    "name": "load_skill",
                    "description": "加载指定 skill 的提示词到系统提示词，参数为 skill 名称列表",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "skill_list": {
    ...
    def load_skill(self,skill_list):#skill_list = []
        skill_manager.load_from_list(skill_list)
        return '已加载' + str(list(skill_manager.available_skill))
...
def res():  
    def build_request_json():
        ...
        json_config = {
                ...
                "messages": mes.info_skill_sys_prmt()     ,#mes.message,#这里还要修改一下，加一个路由，当模型是非视觉的时候切到特殊mes
                "stream": True,
                'tools':tool.tool,
                }
        return load_model_config(json_config,user.current)
    ...
    def parse_stream(response:requests.Response,loading_animate):
        to_sent,pre_status,status = "","None","None"
        #status ：None tool thinking main
        thinking_con = ""
        con = ""
        tc_main = {}
        tc_args =''
        ...
        for line in response.iter_lines():
            ...
            if delta.get("tool_calls"):
                tool_calls = delta.get("tool_calls")[0]
                tc_function = tool_calls.get('function')
                if tool_calls.get('id'):
                    tc_main['id'] = tool_calls.get('id')
                    tc_main['function'] = {'name':tc_function.get("name")}
                tc_args += tc_function.get("arguments") or ''

            if data.get("choices")[0].get('finish_reason') == "tool_calls":
                #拼装toolcalls
                tc_main['function']['arguments'] = tc_args
                tc_main['type']='function'
            ...
            elif delta.get("content") and delta.get('content').strip():
                status = "main🙂"
    ...
    def run():
        loading_animate = Loading()
        loading_animate.start()
        response = submit(build_request_json())

        con,tool_call = parse_stream(response,loading_animate)
        ...
        if tool_call == {}:
            ...
            return 0
        mes.add_assistant_tool([tool_call])

        tool_call_id ,tool_call_res = tool.excute_tool(tool_call)

        mes.add_tool_result(tool_call_id,tool_call_res)
        return run()
    run()
...
#         \n当前模型: \n{user.current}\n""")
    while 1:
        ...
        if u_input == "/m" or u_input == "/model":         #查看可用模型
            ...
            mes.new_mes()
            mes.now_index = mes.index[-1]
            mes.message = mes.data[mes.now_index]
            print('\033[1A\r         \r',flush=True,end = '')
            continue
        

        elif u_input == "/skill":                    # 列出所有 skill
        ...
        #启用skill，在系统提示词展示info
        elif u_input.startswith('/o'):
            if u_input == '/oo':#快捷启动
                skill_manager.on()
                print("\033[7m已启用\033[0m")
                print('\033[3A\r'+3*(40*" "+'\n')+'\033[3A',end = '',flush=True)
                skill_manager.list_skills()
                continue
            
            ensure = input("on \033[2m开启skill\n\033[0moff \033[2m关闭skill\033[0m\n请输入")
            if ensure == 'on':
                skill_manager.on()
                print('\033[3A\r'+3*(40*" "+'\n')+'\033[3A',end = '',flush=True)
                skill_manager.list_skills()
                ...
                skill_manager.off()
                print(f"\033[1mskill \033[7moff\033[0m",flush=True)
            else:
                print('无事发生')
